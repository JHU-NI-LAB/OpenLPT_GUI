...
void sortID(std::vector<int> &sort_index, std::vector<T> const &nums) {
  ...
  // Custom comparator function for sorting indices based on corresponding
  // values
  auto comparator = [nums](size_t i, size_t j) { return nums[i] < nums[j]; };
...
// Calculate median
template <class T> double getMedian(std::vector<T> const &nums) {
  int n = nums.size();
...
void generateCombinations(size_t N, size_t K,
                          std::vector<std::vector<int>> &out);

// Bilinear interpolation
double bilinearInterp(AxisLimit const &grid_limit,
                      std::vector<double> const &value,
                      std::vector<double> const &pt_vec);

// Trilinear interpolation
double triLinearInterp(AxisLimit const &grid_limit,
                       std::vector<double> const &value,
                       std::vector<double> const &pt_vec);
...
Matrix<T> piecewiseProduct(Matrix<T> const &mtx1, Matrix<T> const &mtx2) {
  if ((mtx1.getDimRow() != mtx2.getDimRow()) ||
      (mtx1.getDimCol() != mtx2.getDimCol())) {
...
void gaussForward(Matrix<T> &u_mtx, std::vector<int> &sortedRowIndex) {
  ...
  for (int iter_y = 0; iter_y < n - 1; iter_y++) {
    ...
    std::swap(sortedRowIndex[t],
              sortedRowIndex[iter_y]); // move the row with "biggest pivot"
    ...
    for (int iter_x = iter_y + 1; iter_x < n; iter_x++) {
      ...
      u_mtx(sortedRowIndex[iter_x], iter_y) =
          m; // it should be 0.
             // here is only used to store the multiplier m

      for (int iter = iter_y + 1; iter < n; iter++) {
        u_mtx(sortedRowIndex[iter_x], iter) -=
            m * u_mtx(sortedRowIndex[iter_y], iter);
      }
    }
  }
};
...
template <class T>
Matrix<T> gaussBackward(Matrix<T> &b_mtx, Matrix<T> &u_mtx,
                        std::vector<int> &sortedRowIndex) {
  // Solve Amat * Xmat = Bmat,
  //  where Amat (n*n), Xmat (n*m), Bmat (n*m).
  // In GaussForward, Amat = Lmat * u_mtx,
  ...
  for (int iter = 0; iter < n; iter++) {
    for (int iter_y = 0; iter_y < n - 1; iter_y++) {
      for (int iter_x = iter_y + 1; iter_x < n; iter_x++) {
        b_mtx(sortedRowIndex[iter_x], iter) -=
            (u_mtx(sortedRowIndex[iter_x], iter_y) *
...
template <class T> Matrix<T> gaussInverse(Matrix<T> const &mtx) {
...
template <class T> Matrix<T> detInverse(Matrix<T> const &mtx) {
  if (mtx.getDimRow() != mtx.getDimCol()) {
    std::cerr << "The matrix is not a square matrix!"
              << "There is no inverse." << std::endl;
    throw error_size;
  }
  ...
  if (n == 2) {
    ...
    if (std::fabs(det) < SMALLNUMBER) {
      std::cout << "myMATH::detInverse warning at line " << __LINE__ << ": "
                << "The determinant is too small, " << det << ". "
    ...
    std::cerr << "DetInverse only supports for 2x2 or 3x3 matrix!" << std::endl;
    throw error_size;
  }
};
...
template <class T> T trace(Matrix<T> const &mtx) {
...
bool isLocalMax(Matrix<T> const &mtx, int row_id, int col_id) {
  ...
  // move_row/col: [up, left, down, right]
  ...
  for (int i = 0; i < 4; i++) {
    ...
    T neighbor_val = mtx(row_id + move_row[i], col_id + move_col[i]);

    // For up/left (i=0,1): neighbor must be strictly less than current
    // For down/right (i=2,3): neighbor must be less than or equal to current
    // This breaks ties by keeping only the top-left pixel
    if (i < 2) {
      // up/left: only return false if neighbor is strictly greater
      if (neighbor_val > val) {
        return false;
      }
    } else {
      // down/right: return false if neighbor is greater OR equal (tie-break)
      if (neighbor_val >= val) {
        return false;
      }
    }
...
void polyfit(std::vector<double> &coeff, std::vector<double> const &x,
             std::vector<double> const &y, int order);

// Create 1D Gaussian kernel
std::vector<double> createGaussianKernel(int radius, double sigma);

// calculate crosscorelation at point (cx, cy)
double imgCrossCorrAtPt(const Image &img, const Image &ref_img, double cx,
                        double cy);
...
double computeLowCutKDE(
    const std::vector<double> &x_raw,
    double p = 0.95,            // modal HDR mass (candidate region width)
    double ratio_thresh = 0.06, // density ratio threshold f(L)/f(mode) (smaller
                                // = more conservative)
