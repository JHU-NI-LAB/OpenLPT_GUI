...
private:
    ...
    bool checkMatch(const std::vector<int>& candidate_ids,
                    double&                  out_e_check) const; // out_e_check: the triangulation error obtained during checking

    std::vector<std::vector<int>>
    pruneMatch(const std::vector<std::vector<int>>& match_candidates,
               const std::vector<double>&           e_checks) const; // e_checks is used to prune matches
