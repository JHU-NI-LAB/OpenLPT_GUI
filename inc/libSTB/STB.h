...
public:
    ...
    // FUNCTIONS //
    STB(const BasicSetting& setting, const std::string& type, const std::string& obj_cfg_path);
    ...
    // load tracks, only active long tracks and active short tracks are needed to load
...
private:
    ...
    std::unique_ptr<ObjectConfig> _obj_config; // settings for IPR, Tracking, Shaking for different object types

    std::vector<std::vector<std::unique_ptr<Object3D>>> _ipr_candidate; // objects from IPR for building tracks
    ...
    int findNN (KDTreeObj3d const& tree_obj3d, Pt3D const& pt3d_est, double radius) const;

    // check whether objects found by IPR are repeated with the last point in active long tracks
    std::vector<ObjFlag> checkRepeat(const std::vector<std::unique_ptr<Object3D>>& objs) const;

    // int linkShortTrack (Track<T3D> const& track, std::vector<T3D> const& obj3d_list, int n_iter);
    int linkShortTrack (Track const& track, int n_iter, KDTreeObj3d const& tree_obj3d, KDTreeTrack const& tree_track);
