...
bool BasicSetting::readConfig(const std::string& config_path) {
    ...
    std::getline(parser, line, ','); _frame_start = std::stoi(line);
    std::getline(parser, line, ','); _frame_end   = std::stoi(line);
    parser.clear();
    ...
    // axis limits
    parser.str(lines[line_id++]);
    std::getline(parser, line, ','); _axis_limit.x_min = std::stod(line);
    std::getline(parser, line, ','); _axis_limit.x_max = std::stod(line);
    std::getline(parser, line, ','); _axis_limit.y_min = std::stod(line);
    std::getline(parser, line, ','); _axis_limit.y_max = std::stod(line);
    std::getline(parser, line, ','); _axis_limit.z_min = std::stod(line);
    std::getline(parser, line, ','); _axis_limit.z_max = std::stod(line);
    parser.clear();
    ...
    if (!_output_path.empty()) {
        ...
        if (back != '/' && back != '\\') _output_path.push_back('/');
    }

    // object types
    parser.str(lines[line_id++]);
    while (std::getline(parser, line, ',')) {
        if (!line.empty()) _object_types.push_back(line);
    }
    parser.clear();
    ...
    if (_load_track) {
        std::getline(parser, line, ',');
        if (!line.empty()) _load_track_frame = std::stoi(line);
...
ObjectConfig::readCommonConfig(const std::string& filepath, BasicSetting& settings) {
    std::ifstream file(filepath);
    REQUIRE_CTX(file.is_open(), ErrorCode::IOfailure, "Config Error: Cannot open file ", filepath + "\n");
    ...
    try {
        ...
        if (_stb_param._n_initial_frames < 4) _stb_param._n_initial_frames = 4; //n_initial_frames must be >= 4.
        _stb_param._radius_search_track = std::stod(lines[id++]) * settings._voxel_to_mm;
        ...
        THROW_FATAL_CTX(ErrorCode::IOfailure, "Config Error: Failed to parse common config in ", filepath + "\n");
    }
...
// --------------------------- TracerConfig ---------------------------
bool TracerConfig::readConfig(const std::string& filepath, BasicSetting& settings) {
...
// --------------------------- BubbleConfig ---------------------------
bool BubbleConfig::readConfig(const std::string& filepath, BasicSetting& settings) {
    auto [lines, id] = readCommonConfig(filepath, settings);
...
TracerConfig::creatObject3D(CreateArgs args) const {
    auto up = std::make_unique<Tracer3D>();

    if (args._proto)      up->copyCommonFrom(*args._proto);
    if (args._pt_center)  up->_pt_center = *args._pt_center;
...
// --------------------------- BubbleConfig ---------------------------
std::unique_ptr<Object3D>
BubbleConfig::creatObject3D(CreateArgs args) const {
    ...
    if (up->_r3d <= 0.0 && args._compute_bubble_radius && args._cams && !up->_obj2d_list.empty()) {
        up->_r3d = Bubble::calRadiusFromCams(*args._cams, up->_pt_center, up->_obj2d_list);
    }
    if (up->_r3d <= 0.0) up->_r3d = -1.0;
