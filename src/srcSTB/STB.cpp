...
STB::STB(const BasicSetting& setting, const std::string& type, const std::string& obj_cfg_path)
    : _basic_setting(setting)
{
    ...
    // read configuration settings of IPR, Tracking, Shaking, etc. for the specified object type
    if (type == "Tracer") {
        ...
        THROW_FATAL_CTX(ErrorCode::UnsupportedType, "Unsupported object type:", type);
    }
...
{
    if (frame_id < _basic_setting._frame_start || frame_id > _basic_setting._frame_end)
    {
        THROW_FATAL_CTX(ErrorCode::OutOfRange, "Frame number is out of range", "frame_id=" + std::to_string(frame_id));
    }
    ...
    if (frame_id - _basic_setting._frame_start < _obj_config->_stb_param._n_initial_frames)
    {
        // Initial phase
    ...
    if (frame_id == _basic_setting._frame_end)
    {
        ...
        std::cout << "Number of initial phase frames: " << _obj_config->_stb_param._n_initial_frames << std::endl;
...
void STB::runInitPhase (int frame, std::vector<Image>& img_list)
{
    ...
    IPR ipr(cam_list);
    // IPR
    std::vector<std::unique_ptr<Object3D>> obj3d_list =  ipr.runIPR(*_obj_config, img_list);
    _ipr_candidate.emplace_back(std::move(obj3d_list));

    // Calculate the velocity field if number of processed frames is enough.
    // Predict Field
    if (frame - _basic_setting._frame_start == _obj_config->_stb_param._n_initial_frames - 1)
    {
        std::cout << std::endl;
        ...
        const auto need_len = static_cast<size_t>(_obj_config->_stb_param._n_initial_frames);

        for (auto it = _short_track_active.begin(); it != _short_track_active.end(); ) {
...
void STB::buildTrackFromPredField (int frame_id, const PredField* pf) {
    ...
    if (id_ipr > 0) {// first frame does not have a previous frame
        ...
        for (int j = 0; j < n_sa; j ++)
        {
            ...
            link_id[j] = findNN(tree_obj3d, _short_track_active[j]._obj3d_list.back()->_pt_center + vel_curr, _obj_config->_stb_param._radius_search_obj); 
        }
        // update short track and _is_tracked status for the next frame
        for (int j = n_sa - 1; j > -1; j --) // reverse order to avoid invalid index after erasing
        {
    ...
    {
        if (_ipr_candidate[id_ipr][i]                   //if the object has been moved, _ipr_candidate[id_ipr][i] will null_ptr
...
std::unique_ptr<Object3D> STB::predictNext(const Track& tr) const
{
    ...
    // Create a same-kind object seeded from the last one (copy type-specific params)
    CreateArgs a;
    ...
    for (int axis = 0; axis < 3; ++axis)
    {
        ...
        int iter = 0;
        while (std::fabs(error) > SMALLNUMBER && iter < WIENER_MAX_ITER && step > 0.0) {
            prediction = 0.0;
            for (int j = 0; j < order; ++j) {
        ...
        if (order >= 1 && step == 0.0) {
            ...
            prediction = series[static_cast<size_t>(order)]
                       + (series[static_cast<size_t>(order)] - series[static_cast<size_t>(order - 1)]);
        } else {
            prediction = 0.0;
            for (int j = 0; j < order; ++j)
                prediction += filter[static_cast<size_t>(j)] * series[static_cast<size_t>(j + 1)];
        }

        if (shifted) prediction -= kShift;
...
int STB::findNN(KDTreeObj3d const& tree_obj3d, Pt3D const& pt3d_est, double radius) const
{
    ...
    tree_obj3d.findNeighbors(resultSet, pt3d_est.data(), nanoflann::SearchParameters());
...
void STB::runConvPhase (int frame, std::vector<Image>& img_list)
{
    ...
    for (int i = 0; i < n_la; i ++)
    {
        ...
        if (is_inRange[i])
        {
            is_inRange[i] = obj3d_list_pred[i]->isReconstructable(_basic_setting._cam_list);
        }
    }
    ...
    if (obj3d_list_pred.size() > 0)
    {
        ...
        {
            const bool ok = (flags[read] == ObjFlag::None); // neither ghost nor repeated
            if (ok)
            {
                // add to the corresponding track
                _long_track_active[read].addNext(std::move(obj3d_list_pred[read]), frame);

                // move the track to the right postion if some tracks ahead it was rmoved
                if (write != read)
    ...
    for (size_t i = 0; i < obj3d_list.size(); ++i) {
        if (is_repeate[i] == ObjFlag::None) kept.emplace_back(std::move(obj3d_list[i]));
    }
    ...
    if (n_obj3d > 0)
    {
        ...
        for (int i = 0; i < n_sa; ++i)
        {
            ...
            {
                // Not linked this round: drop this short track (do not keep, do not promote)
                ++rm_sa;   // accounting: processed one short-active
    ...
    // Serial phase: in-place compaction + move erased ones to inactive if long enough
    size_t write = 0;
    ...
    std::cout << " Pruning time: "
            << static_cast<double>(t_end - t_start) / CLOCKS_PER_SEC
            << " s" << std::endl;
...
std::vector<ObjFlag> STB::checkRepeat(const std::vector<std::unique_ptr<Object3D>>& objs) const
{
    ...
    if (last_centers.empty()) return flags;
...
int STB::linkShortTrack(Track const& track,
                        int n_iter,
                        ...
                        KDTreeTrack const& tree_track)
{
    ...
    for (int step = 0; step < n_iter; ++step)
    {
        ...
        nanoflann::RadiusResultSet<double,size_t> result_set(r_track2, indices_dists);
        ...
        if (!indices_dists.empty())
        {
            ...
            double mean_d2 = 0.0;
            for (const auto& it : indices_dists) mean_d2 += it.second;
            ...
            for (const auto& it : indices_dists)
            {
                ...
                if (idx >= _long_track_active.size()) continue;
                const auto& tr = _long_track_active[idx];
                const size_t len = tr._obj3d_list.size();
                if (len < 2) continue;
                if (!tr._obj3d_list[len-1] || !tr._obj3d_list[len-2]) continue;
            ...
            if (sum_w > 0.0) {
                ...
                // No valid neighbor displacements (e.g., all had len<2): fallback to zero velocity.
                // The object-association radius below still expands with 'factor'.
                vel = Pt3D{0,0,0};
            }
        }
        else
        ...
        // If there were zero neighbors, you may also try the zero-velocity point 'p_last'
        // once more with the same r_obj to be symmetric with legacy behavior:
        //
        if (indices_dists.empty()) {
            obj3d_id = findNN(tree_obj3d, p_last, r_obj);
            if (obj3d_id != UNLINKED) break;
        }
    }
...
bool STB::checkLinearFit(Track const& track)
{
    ...
    int n_pts =  n_initPhase > 4 ? 4 : n_initPhase; // only select tha last four points at maximum.

    if (len < n_pts) return false;
...
void STB::saveTracksAll(const std::string& folder, int frame)
{
    ...
    // Helper: remove all files in 'dir' that start with the given prefix and end with ".csv"
    ...
    saveTracks((dir / ("LongTrackActive_"    + s + ".csv")).string(), _long_track_active);
    saveTracks((dir / ("ShortTrackActive_"   + s + ".csv")).string(), _short_track_active);
...
void STB::saveTracks (std::string const& file, std::deque<Track>& tracks)
{
    ...
    REQUIRE_CTX(output.is_open(), ErrorCode::IOfailure, "Cannot open file:", file);
    ...
    switch (kind) {
        ...
        case ObjectKind::Bubble:
            output << ",R3D";
            for (int i = 0; i < n_cam; ++i) {
                output << ",cam" << i << "_x(col),cam" << i << "_y(row),cam" << i << "_rpx";
            }
            break;
        default:
...
void STB::loadTracks(const std::string& file, std::deque<Track>& tracks)
{
    ...
    REQUIRE_CTX(fin.is_open(), ErrorCode::IOfailure, "LoadTracks: cannot open file:", file);
    ...
    while (true) {
        ...
        std::string s_tid;
        if (!std::getline(row, s_tid, ',')) continue;

        int tid = -1;
        try { tid = std::stoi(s_tid); } catch (...) { continue; } // try to get track ID.

        // Rewind this line so Track::loadTrack can process it as the first row of that track.
        fin.clear();
        fin.seekg(pos);
...
void STB::loadTracksAll(std::string const& folder, int frame)
{
    ...
    struct Item { const char* prefix; std::deque<Track>& dst; } items[] = {
    ...
    for (auto& it : items) {
        ...
        REQUIRE_CTX(probe.is_open(), ErrorCode::IOfailure, "LoadTracks: missing file:", path);

        probe.close();

        loadTracks(path, it.dst); // dst has _long_track_active and _short_track_active
    }
}
