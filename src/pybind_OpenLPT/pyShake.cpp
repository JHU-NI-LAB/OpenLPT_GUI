...
struct DebugAccess_Shake {
    ...
    static void calResidueImage(Shake& s,
    ...
    static PixelRange calROIBound(const Shake& s, int id_cam, double xc, double yc, double dx, double dy)
    { return s.calROIBound(id_cam, xc, yc, dx, dy); }

    static std::vector<ROIInfo> buildROIInfo(const Shake& s, const Object3D& obj, const std::vector<Image>& img)
...
void bind_Shake(py::module_& m) {
    ...
    py::class_<ROIInfo>(m, "ROIInfo")
        ...
        .def("getAugImg",    &ROIInfo::getAugImg, py::return_value_policy::reference_internal)
        .def("inRange",      &ROIInfo::inRange, py::arg("row"), py::arg("col"))
        .def("mapToLocal",   [](const ROIInfo& r, int row, int col){
                int i=0, j=0;
                bool ok = r.mapToLocal(row, col, i, j);
                return py::make_tuple(ok, i, j);
            }, py::arg("row"), py::arg("col"))
        // 单像素访问（通常不建议在 Python 逐像素改写；需要的话可以包块操作）
        .def("aug_img",      (double& (ROIInfo::*)(int,int)) &ROIInfo::aug_img, py::return_value_policy::reference_internal)
        .def("aug_img",      (const double& (ROIInfo::*)(int,int) const) &ROIInfo::aug_img, py::return_value_policy::reference_internal)
        .def("corr_map",     (double& (ROIInfo::*)(int,int)) &ROIInfo::corr_map, py::return_value_policy::reference_internal)
        .def("corr_map",     (const double& (ROIInfo::*)(int,int) const) &ROIInfo::corr_map, py::return_value_policy::reference_internal)
        .def_readwrite("ROI_range", &ROIInfo::_ROI_range);

    // -------- 策略基类与派生（如果不在 Python 里继承/覆盖，trampoline 不是必须）--------
    ...
    py::class_<Shake>(m, "Shake", py::dynamic_attr())
        ...
        // .def(py::init<const std::vector<Camera>&, const ObjectConfig&>(),
        //      py::arg("cams"), py::arg("obj_cfg"), py::keep_alive<1,2>(), py::keep_alive<1,3>())
        ...
        // 改为 Python 传 list[Object3D]（或其派生），C++ 里深拷贝成 unique_ptr 容器再调用
        .def("runShake",
            [](Shake& s,
                const std::vector<Object3D*>& objs_py,
                const std::vector<Image>&     img_orig) {
                auto objs = make_unique_obj3d_list(objs_py); // deep copy, update of objs won't refect in objs_py
                auto flags = s.runShake(objs, img_orig);
                // return the objs to interface
                py::list out(objs.size());
                size_t i = 0;
                for (auto& up : objs) {
                    if (up) out[i++] = py::cast(std::move(up));  
                    else    out[i++] = py::none();
                }
                return py::make_tuple(flags, out);
            },
            py::arg("objs").noconvert(),
            py::arg("img_orig").noconvert()
        )
        ...
        .def("calResidualImage", [](Shake& s,
                ...
                return s.calResidualImage(
                    reinterpret_cast<const std::vector<std::unique_ptr<Object3D>>&>(objs),
                    img_orig, flags_ptr
                );
            }, 
            py::arg("objs"), 
        ...
        .def_property_readonly("cams", [](Shake& s) -> const std::vector<Camera>& {
                return DebugAccess_Shake::cams(s);
            }, py::return_value_policy::reference_internal)
        .def_property_readonly("obj_cfg", [](Shake& s) -> const ObjectConfig& {
                return DebugAccess_Shake::obj_cfg(s);
            }, py::return_value_policy::reference_internal)
        .def_property_readonly("img_res_list",
            [](Shake& s) -> std::vector<Image>& { return DebugAccess_Shake::img_res_list(s); },
            //[](Shake& s, const std::vector<Image>& v){ DebugAccess_Shake::img_res_list(s) = v; },
            py::return_value_policy::reference_internal)
        .def_property("score_list",
            [](Shake& s) -> std::vector<double>& { return DebugAccess_Shake::score_list(s); },
            [](Shake& s, const std::vector<double>& v){ DebugAccess_Shake::score_list(s) = v; },
            py::return_value_policy::reference_internal)
        ...
        .def(
            ...
            [](Shake& s,
                ...
                auto objs = make_unique_obj3d_list(objs_py);
                const std::vector<ObjFlag>* flags_ptr = flags_opt ? &*flags_opt : nullptr;
            ...
            py::arg("non_negative") = false,
            py::arg("flags") = py::none()
            )

        .def("calROIBound", [](const Shake& s, int id_cam, double x_center, double y_center, double dx, double dy) {
                return DebugAccess_Shake::calROIBound(s, id_cam, x_center, y_center, dx, dy);
            }, py::arg("id_cam"), py::arg("x_center"), py::arg("y_center"), py::arg("dx"), py::arg("dy"))

        .def("buildROIInfo", [](const Shake& s, const Object3D& obj, const std::vector<Image>& img) {
                return DebugAccess_Shake::buildROIInfo(s, obj, img);
            }, py::arg("obj"), py::arg("img_orig"))

        .def("shakeOneObject", [](const Shake& s, Object3D& obj, std::vector<ROIInfo>& roi, double delta, const std::vector<bool>& shake_cam) {
                CreateArgs args;
                args._proto = &obj;
                std::unique_ptr<Object3D> obj_shaked = DebugAccess_Shake::obj_cfg(const_cast<Shake&>(s)).creatObject3D(std::move(args));
                double residue = DebugAccess_Shake::shakeOneObject(s, *obj_shaked, roi, delta, shake_cam);
